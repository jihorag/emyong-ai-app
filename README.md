...
```
화면 26개 렌더 · localStorage 키 9개 · AI 프롬프트 6종 바이트 대조
· 콘솔 에러 0 · 대시보드 수치 반응
```
...
> **지금은 기본값이 켜짐입니다.** 시연 기간이라 처음 들어오면 데모 데이터로 시작합니다.
> 끄려면 **⚙️ 설정 › 데모 모드** 를 끄거나 주소에 `?demo=0` 을 붙이세요. 한 번 끄면 계속 꺼져 있습니다.

시연용입니다. 26일치 학습 기록이 채워지고, AI 응답을 서버·API 키 없이 기기 안에서 만듭니다.
...
PWA는 서비스워커가 옛 번들을 붙들기 때문에, 배포해도 휴대폰에 예전 화면이 계속 떴습니다.
지금은 빌드마다 `version.json` 에 빌드 ID가 박히고, 앱이 이걸 지켜봅니다.

| 언제 확인하나 | |
|---|---|
| 앱을 열 때 | 매번 |
| 화면으로 돌아올 때 | 탭 전환·포커스·bfcache 복귀 모두 |
| 네트워크가 돌아올 때 | |
| 열어 둔 채로 | 3분마다 |

빌드 ID가 다르면 **캐시를 전부 비우고 서비스워커까지 해제한 뒤 새로고침**합니다.
같은 버전이면 아무 일도 일어나지 않습니다(무한 새로고침 방지).

> 이 장치가 없던 빌드를 쓰는 기기는 한 번 더 앱을 열어야 새 코드를 받습니다. 그 뒤부터 자동입니다.
